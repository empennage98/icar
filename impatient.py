    ...
    for gamma in [0.05, 0.02, 0.01]:
      for only_car in [True, False]:
        ...
        all_total_runtimes.append((settings, icar.final_precheck_accepted, 
          ...
          ret))
        print('global_t: {}'.format(icar.global_t))
        for k1, x in icar.env.ran_so_far.items():
          assert type(k1) == int
          for k2, y in x.items():
            assert type(k2) == int
            if type(y) not in [int, float]:
              ...
              pdb.set_trace()
        #pdb.set_trace()
        with open('reg_icar_rsf_only_car{}.dump'.format(only_car), 'wb') as f:
          pickle.dump(dict(icar.env.ran_so_far.copy()), f)

        # print('-------------------- car only --------------')
        # settings2 = settings.copy()
        # settings2['only_car'] = True
        # icar = ImpatientCapsAndRuns(**settings2)
        # icar.run()
        # print('global_t car_only: {}'.format(icar.global_t))
        # del icar

      exit(0)

      print(all_total_runtimes)
      with open('reg_icar_three_epsilon{}_delta{}_gamma{}_r{}_p{}_seed{}.dump'.format(settings['epsilon'],delta,gamma,520+seed), 'wb') as f:
